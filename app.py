...
def main():
    ...
    if selected=="Private LLM":
        ...
        if userChoise=="Ingest/Train new documents":
            ...
            with modelCol2:
                uploaded_file = st.file_uploader("Upload your new training documents",type=["pdf", "doc", "txt","csv","ppt","pptx","html","json"],help="Scanned documents are not supported yet!", accept_multiple_files=True)
        ...
        if userChoise=="Query from existing knowledge base":
            userChoise1Col1,userChoise1Col2,userChoise1Col3 =st.columns([60,80,60])
                    ...
                    with textcontainer:
                        ...
                        if query:
                            with st.spinner("typing..."):
                                ...
                                # print(context)  
                                response = llamaMain('cpu',True,query)
    ...
    if selected=="Public LLM":
        ...
        with choiseCo2:
            userChoise = st.radio("",options=["Query from existing knowledge base", "Ingest/Train new documents"],index=0,horizontal=True)
        if userChoise=="Ingest/Train new documents":
            modelCol1,modelCol2,modelCol3 =st.columns([40,80,40])
            # with modelCo1:
            #     modelOpt=st.selectbox(label="Select Model",options=["","GPT4All","Llama-7B","Llama-13B","Llama-70B"])
            #     if modelOpt:
            #         st.write(f"The Model opted for Private training is {modelOpt}")
